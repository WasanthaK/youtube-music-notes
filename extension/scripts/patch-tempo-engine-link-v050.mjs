...
r('    model: guitarEar.model,','    model: guitarEar.model,\n    phase: guitarEar.phase ?? null,\n    inferenceMode: guitarEar.inferenceMode ?? null,\n    rhythmConditioning: guitarEar.rhythmConditioning ?? null,\n    modelSha256: guitarEar.modelSha256 ?? null,\n    graphOptimizationLevel: guitarEar.graphOptimizationLevel ?? null,\n    modelFallbackReason: guitarEar.modelFallbackReason ?? null,','compact Phase-2e metadata');
r("? 'guitar-ear-v0.2d+basic-pitch-ensemble-v2-browser'", "? (guitarEar?.phase === 'phase2e-rhythm'\n        ? (rhythmic.stats.enabled ? 'guitar-ear-v0.2e-rhythm+basic-pitch-ensemble-v3-browser-tempo-v1' : 'guitar-ear-v0.2e-rhythm+basic-pitch-ensemble-v3-browser')\n        : (rhythmic.stats.enabled ? 'guitar-ear-v0.2d+basic-pitch-ensemble-v2-browser-tempo-v1' : 'guitar-ear-v0.2d+basic-pitch-ensemble-v2-browser'))",'engine');
fs.writeFileSync(p,t);
for(const f of ['manifest.json','package.json']){const d=JSON.parse(fs.readFileSync(f,'utf8'));d.version='0.5.2';fs.writeFileSync(f,JSON.stringify(d,null,2)+'\n');}
