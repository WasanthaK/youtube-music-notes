...
export function buildGuitarTranscription(frames, onsets, contours) {
  ...
  clusters.forEach((cluster, chordId) => {
    ...
    notes.forEach(note => playable.push({ ...note, chordId, bendSemitones: bendSemitones(note.pitchBends) }));
  });
  ...
  const averageConfidence = playable.length ? playable.reduce((sum, note) => sum + note.confidence, 0) / playable.length : 0;
