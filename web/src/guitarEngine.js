...
function assignClusterToGuitar(cluster, previousHandPosition, v12 = false) {
  ...
  if (!solution) return { notes: [], handPosition: previousHandPosition, rejected: originalCount };
...
function assignClusters(clusters, v12 = false) {
  ...
  clusters.forEach((cluster, chordId) => {
    ...
    result.notes.forEach(note => playable.push({ ...note, chordId, bendSemitones: bendSemitones(note.pitchBends) }));
  });
...
function applyLearnerDensityGuard(notes) {
  ...
  return { notes: notes.filter(note => !rejected.has(note)), rejected: rejected.size };
}
...
function confidenceStats(notes) {
  return {
    ...
    averageConfidence: notes.length ? notes.reduce((sum, note) => sum + note.confidence, 0) / notes.length : 0,
  };
}
